...
picked flag to indicate if it has been collected
...
Environment objects (e.g., doors) are also stored as objects keyed by ID.
...
Demonstrate a full reset of the system

Assumptions

The game contains a single player

Only one simple environment object (door) is modeled

No rendering or UI is required

Focus is strictly on state management and logic